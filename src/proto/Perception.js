...
export class Perception {
  ...
  categorizeSensoryInput(sensoryInput) {
    if (typeof sensoryInput !== 'object' || sensoryInput === null) {
      throw new TypeError('Invalid sensory input: expected an object, received ' + typeof sensoryInput);
    }
  ...
  process(data) {
    if (!data) {
      throw new TypeError('Data cannot be null or undefined. Received: ' + data);
    }
  ...
  processMultiple(inputs) {
    if (!Array.isArray(inputs)) {
      throw new TypeError('Inputs must be an array. Received: ' + typeof inputs);
    }
